...
// Присваиваем указателям начало каждой строки статического массива
void transform(int *buf, int **matr, int n, int m)
{
    for (int i = 0; i < n; i++)
    {
...
// Формируем картинку, используя предопределённые массивы с данными
void make_picture(int **picture, int n, int m)
{
    // Статические массивы для частей картинки
    ...
    // Листья деревьев - два направления
    for (int p = 0; p < 2; p++)
    {
...
// Вывод матрицы с разделением пробелами и без лишних пробелов/переносов
void output(int **matrix, int n, int m)
{
    for (int i = 0; i < n; i++)
    {
